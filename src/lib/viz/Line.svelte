<script lang="ts">
	...
	export let data: Record<string, { x: number; y: number }[]> = {};
	export let labels: string[];
	export let colors: string[] = [];
	// TODO: export let format = 'currency';
	$: console.log(data);
	...
	let y1 = Infinity;
	let y2 = -Infinity;
	let all_data: { x: number; y: number }[] = [];
	...
	<Pancake.Chart {x1} {x2} {y1} {y2}>
		...
		{#if closest}
			<Pancake.Point x={closest.x} y={closest.y}>
				...
				>
					<span class="text-sm">{format_x_label(closest.x)}</span>
					<strong class="text-secondary-content">Hits: </strong><span class="text-secondary-content"
						>{closest.y}</span
					>
				</div>
			</Pancake.Point>
